...
def get_song_comments(cookies, headers, song_id, offset=0, limit=20):
    ...
    try:
        response = requests.post(
            url, data=encrypted_data, headers=headers, cookies=cookies, timeout=10
        )
...
if __name__ == "__main__":
    song_information = pd.read_csv(
        "/home/python_crawler/music_website/data/song_information_new.csv"
    )
    ...
    for song_id, song_name in tqdm(zip(song_ids, song_names)):
        comment_json = get_song_comments(
            headers=base_headers, cookies=cookies, song_id=song_id
        )
        if "hotComments" in comment_json["data"]:
            ...
            if not comment_json["data"]["totalCount"]:
                ...
                comments_counts.append(
                    {"song_name": song_name, "song_id": song_id, "total_count": 0}
                )
            else:
                comments_counts.append(
                    {
        ...
        comments_information_df.to_csv(
            "/home/python_crawler/data/comments_information_2.csv", index=True
        )
        comments_counts_df.to_csv(
            "/home/python_crawler/data/comments_counts_2.csv", index=True
        )
