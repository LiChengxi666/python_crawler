...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        # 导入歌手数据, 使用id做唯一标识
        artist_csv_path = (
            "/home/python_crawler/music_website/data/artist_information.csv"
        )
            ...
            with open(artist_csv_path, "r", encoding="utf-8") as file:
                ...
                for row in reader:
                    ...
                    if created:
                        artists_created += 1
                        self.stdout.write(
                            f"Created artist: {artist.name}, totally: {artists_created}"
                        )
            ...
            with open(song_csv_path, "r", encoding="utf-8") as file:
                ...
                for row in reader:
                    ...
                    if created:
                        songs_created += 1
                        self.stdout.write(
                            f"Created song: {song.title}, totally: {songs_created}"
                        )
